  ...
  return (
    <div className="mb-40">
      {routines.map((routine, idx) => {
        return (
          <div className="mb-4" key={idx}>{routine}</div>
        )
      })}
    </div>
  )
...
const MyContent = ({ tab }: MyContentProps) => {
  ...
  switch (tab) {
    case 'a': return <MyRoutine />
    case 'b': return <FavoriteRoutine />
    case 'c': return <ParticipatedRoutine />
    case 'd': navigate('/routine/make')
  }
  ...
  return (
    <div className="flex">
      <nav className="flex flex-col content-center text-lg mt-10 ml-20">
        ...
        <button value="d" onClick={handleClick} className={`px-10 py-5 my-5${tab === 'd' ? ' bg-orange text-white-100 font-bold rounded-full' : ''}`}>
          밀리루틴 개설하기
        </button>
      </nav>
